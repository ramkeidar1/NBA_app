...
class FormEvalJSON(BaseModel):
    model_config = ConfigDict(populate_by_name=True, alias_generator=_to_kebab)
    ...
    seed: int = Field(
        description="The team's current league seed/position (e.g., '4').")
    offensive_rating: float = Field(
        description="The full-season offensive efficiency rating (points produced per 100 possessions).")
    defensive_rating: float = Field(
        description="The full-season defensive efficiency rating (points allowed per 100 possessions).")
    rating_differential: float = Field(
        description="The full-season net efficiency differential, calculated as (Offensive Rating minus Defensive Rating).")
