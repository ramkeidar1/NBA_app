...
class FormAgentSchema(BaseModel):
    ...
    expected_loser_form: str = Field(
        ..., alias="Expected losser Form"
    )  # Catches typo
...
class MatchupAgentSchema(BaseModel):
    ...
    differential_net: str = Field(..., alias="Deferntial net")  # Catches typo
    interesting_matchups: str = Field(
        ..., alias="Intersting Matchups"
    )  # Catches typo
    certainty: int = Field(..., alias="Certainty", ge=0, le=100)
...
class GameAnalysisSchema(BaseModel):
    ...
    name: str  # Always mapped during data-transformation step
